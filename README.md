...
| **Skills** | 10 auto-loaded skills: framework best practices + domain workflows | `skills/` · `template/.claude/skills/` |
...
### The 10 skills (details [below](#bundled-skills--companion-plugins))

`frontend-design` · `next-best-practices` (+20 refs) · `playwright-best-practices` (~60 refs) · `e2e-flow` · `worktree-dev` · `roster-import` · `firestore-config-edit` · `firebase-best-practices` (+8 refs) · `payment-integration` (+6 refs) · `conventions` — auto-loaded by Claude when the task matches their triggers.
